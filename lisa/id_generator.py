...
class MetadataID:
    ...
    def __repr__(self):
        return f"MetadataID(requirement_documentation_key={self.requirement_documentation_key}, id={self.id})"
    
    def __eq__(self, other):
        return isinstance(other, MetadataID) and self.requirement_documentation_key == other.requirement_documentation_key and self.id == other.id

    def __hash__(self):
        return hash((self.requirement_documentation_key, self.id))
    
    @classmethod
    def from_str(cls, id_str: str) -> "MetadataID":
        try:
...
class RequirementID:
    ...
    def __str__(self):
        return f"{self.requirement_documentation_key}-{self.metadata_key}-{self.id}"

    def __repr__(self):
        return f"RequirementID(requirement_documentation_key={self.requirement_documentation_key}, metadata_key={self.metadata_key}, id={self.id})"

    def __eq__(self, other):
        return isinstance(other, RequirementID) and self.requirement_documentation_key == other.requirement_documentation_key and self.metadata_key == other.metadata_key and self.id == other.id

    def __hash__(self):
        return hash((self.requirement_documentation_key, self.metadata_key, self.id))
    
    @classmethod
    def from_str(cls, id_str: str) -> "RequirementID":
        try:
            head, id_part = id_str.rsplit('-', 1)
            requirement_documentation_key_str, metadata_key = head.split('-', 1)
            requirement_documentation_key = RequirementDocumentationID.from_str(requirement_documentation_key_str)
            id = int(id_part)
            return cls(requirement_documentation_key=requirement_documentation_key, metadata_key=metadata_key, id=id)
        except ValueError:
            raise ValueError(f"Invalid ID string: {id_str}. Expected format: <RequirementDocumentationID>-<metadata_key>-<id>.")
...
class IDGenerator:
    ...
    def generate_metadata_id(self, filename: str, metadata_key: str) -> MetadataID:
        requirement_documentation_key = self.generate_requirement_documentation_id(filename)
        return MetadataID(requirement_documentation_key=requirement_documentation_key, id=metadata_key)
    
    def generate_requirement_id(self, metadata_id: MetadataID, requirement_key: int) -> RequirementID:
        requirement_documentation_key = metadata_id.requirement_documentation_key
        metadata_key = metadata_id.id
        return RequirementID(requirement_documentation_key=requirement_documentation_key, metadata_key=metadata_key, id=requirement_key)
